...
{
    LOGD("%s:%d creating vsync thread", __FUNCTION__,__LINE__);
    int return_value = pthread_create(&dev->vsync_thread, NULL, hwc_vsync_thread, dev);
    if (return_value) {
    LOGE("%s:%d failed to start vsync thread: %s", __FUNCTION__,__LINE__ ,strerror(return_value));
        ...
        size_t numDisplays, hwc_display_contents_1_t** displays)
{
    ...
    if(t1.tv_sec == ctx->logtime) {
        ...
        LOGV("FrameBuffer [%d] reloads", ctx->ucount);
        ctx->ucount = 1;

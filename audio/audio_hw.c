...
int select_volume(int volume)
{
   ...
   if (ret == -1)
      LOGE("%s: ioctl %d failed", PCG_IOCTL_DEVICE, PCG_IOCTL_SETVOL);
...
static void select_devices(struct audio_device *adev)
{
    ...
    LOGV("current device=%d, last device=%d", adev->current_device, adev->last_device);
    ...
    LOGV("current device updated to: %d", adev->last_device);
}
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ...
    LOGV("voice volume: %.1f, %d", volume, scaledvol);
